...
Grouping of Modbus registers for **optimized polling** and **batch operations**. The firmware reads multiple consecutive Modbus addresses in a single command when parameters are grouped together.

### How Packet Calculation Works

**Automatic Calculation:**
1. Click **"🔄 Calculate Packets"** button in toolbar
2. Algorithm groups parameters by:
   - Same Slave ID + Function Code
   - Sequential or nearby addresses
   - Within 70 address span limit
3. Calculates three values per packet:
   - **Packet #**: Sequential packet number (1-based)
   - **Packet Start (SA)**: Minimum address in packet
   - **Packet Regs (NRT)**: Span of addresses to read

**Firmware Constraints:**
- ✅ Maximum 70 registers per packet
- ✅ **Maximum 70 address span per packet** (critical!)
- ✅ Same Slave ID per packet
- ✅ Same Function Code per packet
- ✅ Write operations (FC 5,6,15,16) are individual packets
- ✅ Read operations (FC 1,2,3,4) are grouped for efficiency

**Example:**
```
Parameters at addresses: [32, 33, 35, 38, 39, 41]
Slave ID: 4, FC: 3

→ Packet #: 1
→ Packet Start (SA): 32 (minimum address)
→ Packet Regs (NRT): 10 (41 - 32 + 1 = 10 addresses)
→ Firmware Command: READ_HOLDING_REGISTERS(slave=4, addr=32, count=10)
   (Reads addresses 32-41, even though 34, 36, 37, 40 are unmapped)
```
...
|-------|-------------|---------|-----------------|
| **Packet #** | Packet identifier | 1, 2, 3 | ✅ Yes |
| **Packet Start (SA)** | First Modbus address to read | 32, 1561 | ✅ Yes |
| **Packet Regs (NRT)** | Number of addresses to read | 10, 6, 1 | ✅ Yes |
| **Param Type** | write / feedback / read_only | write | ❌ Manual |
| **Paired With** | Link write ↔ feedback | 5 (Param ID) | ❌ Manual |
| **JKA Index** | Equipment group index | 0, 1, 2, -1 | ❌ Manual |

### Manual Packet Editing (Advanced)

**When to use:**
- Custom grouping requirements
- Testing specific packet configurations
- Overriding automatic calculation

**How to edit:**
1. Double-click any parameter row
2. Edit **Packet #** field
3. Click **"Save Changes"**
4. **Important:** Re-click **"🔄 Calculate Packets"** to recalculate SA and NRT
5. Validation prevents:
   - Mixed Slave IDs in same packet
   - Mixed Function Codes in same packet
   - Address span > 70
   - More than 70 parameters per packet
